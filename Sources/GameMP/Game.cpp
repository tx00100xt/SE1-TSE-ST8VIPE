...
/*extern FLOAT gam_afAmmoQuantity[6]        = {2.0f, 2.0f,  1.0f, 1.0f, 1.0f, 1.0f };
extern FLOAT gam_afDamageStrength[6]      = {3.5f, 3.0f, 1.75f, 1.5f, 2.0f, 1.0f };
extern FLOAT gam_afEnemyAttackSpeed[6]    = {1.5f, 1.5f, 1.5f, 1.75f, 2.0f, 3.0f };
extern FLOAT gam_afEnemyMovementSpeed[6]  = {1.5f, 1.75f, 2.0f, 2.25f, 2.5f, 3.5f};*/

extern FLOAT gam_afAmmoQuantity[5]        = {2.0f,  2.0f,  1.0f, 1.0f , 1.0f };
extern FLOAT gam_afDamageStrength[5]      = {3.0f,  3.0f,  2.25f, 2.0f , 1.5f };
extern FLOAT gam_afEnemyAttackSpeed[5]    = {0.75f,  0.75f,  1.0f, 2.0f , 2.0f };
extern FLOAT gam_afEnemyMovementSpeed[5]  = {1.0f , 1.0f , 1.0f, 1.25f, 1.5f};
...
// extra enemy multiplier
extern INDEX gam_bExtraEnemiesPerPlayer = FALSE;
extern FLOAT gam_fExtraEnemiesPerPlayerRatio = 0.5f;
extern INDEX gam_iEnemyMultiplier = 2;
extern INDEX gam_iEnemyMax = 100;
extern FLOAT gam_fEnemySpeedAdjuster = 1.0f;
extern FLOAT gam_fPlayerDamageAdjuster = 1.0f;
extern FLOAT gam_fGibLife = 1.0f;

// general game options
extern INDEX gam_bDamageSelf = TRUE;
extern FLOAT gam_fSpeedAdjuster = 1.35f;
extern INDEX gam_bControlRunning = FALSE;
extern INDEX gam_bBackwardsGame = FALSE;
extern FLOAT gam_fWeaponsFireRate = 1.0f;
extern FLOAT gam_fWeaponsDamage = 1.0f;
extern FLOAT gam_fProjectileSpeed = 1.0f;
extern INDEX gam_bSkipLuxor = TRUE;
extern INDEX gam_bSkipSewers = TRUE;
extern INDEX gam_bRandomLevels = FALSE;
extern INDEX gam_bControlFlying = TRUE;
extern INDEX gam_iPostTGPLevel = 0;
extern INDEX gam_bUsePEHealth = TRUE;
/*extern INDEX gam_iMaxHealth = 200;
extern INDEX gam_iTopHealth = 100;
extern INDEX gam_iMaxArmor = 200;
extern INDEX gam_iTopArmor = 100;*/

// weapons options
extern INDEX gam_bGiveAllWeapons = TRUE;
extern INDEX gam_bMaskChainsaw = FALSE;
extern INDEX gam_bMaskColt = FALSE;
extern INDEX gam_bMaskSingleShotgun = FALSE;
extern INDEX gam_bMaskSniper = FALSE;
extern INDEX gam_bMaskFlamer = FALSE;
extern INDEX gam_bMaskTommyGun = FALSE;
extern INDEX gam_bMaskRocketLauncher = FALSE;
extern INDEX gam_bMaskGrenadeLauncher = FALSE;
extern INDEX gam_bMaskMiniGun = FALSE;
extern INDEX gam_bMaskLaser = FALSE;
extern INDEX gam_bMaskCannon = FALSE;
extern INDEX gam_bMaskTeleGun = FALSE;

// bot options
extern INDEX gam_bUseBots = FALSE;
extern INDEX gam_iBotsPerPlayer = 1;
extern FLOAT gam_fBotSkill = 3.0f;

// spawner battery options
extern INDEX gam_bTestSpawnerBatteries = FALSE;

// single player chit
extern INDEX gam_iFPS = 0;
...
void CGame::InitInternal( void)
{
  ...
  _pShell->DeclareSymbol("persistent user INDEX gam_bExtraEnemiesPerPlayer;",(void *)&gam_bExtraEnemiesPerPlayer);
  _pShell->DeclareSymbol("persistent user FLOAT gam_fExtraEnemiesPerPlayerRatio;", (void *)&gam_fExtraEnemiesPerPlayerRatio );
  _pShell->DeclareSymbol("persistent user INDEX gam_iEnemyMultiplier;",      (void *)&gam_iEnemyMultiplier      );
  _pShell->DeclareSymbol("persistent user INDEX gam_iEnemyMax;",             (void *)&gam_iEnemyMax             );
  _pShell->DeclareSymbol("persistent user INDEX gam_bDamageSelf;",           (void *)&gam_bDamageSelf           );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fSpeedAdjuster;",        (void *)&gam_fSpeedAdjuster        );
  _pShell->DeclareSymbol("persistent user INDEX gam_bControlRunning;",       (void *)&gam_bControlRunning       );
  _pShell->DeclareSymbol("persistent user FLOAT gam_bBackwardsGame;",        (void *)&gam_bBackwardsGame        );
  _pShell->DeclareSymbol("persistent user INDEX gam_bGiveAllWeapons;",       (void *)&gam_bGiveAllWeapons       );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskChainsaw;",         (void *)&gam_bMaskChainsaw         );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskColt;",             (void *)&gam_bMaskColt             );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskSingleShotgun;",    (void *)&gam_bMaskSingleShotgun    );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskSniper;",           (void *)&gam_bMaskSniper           );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskFlamer;",           (void *)&gam_bMaskFlamer           );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskTommyGun;",         (void *)&gam_bMaskTommyGun         );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskRocketLauncher;",   (void *)&gam_bMaskRocketLauncher   );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskGrenadeLauncher;",  (void *)&gam_bMaskGrenadeLauncher  );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskMiniGun;",          (void *)&gam_bMaskMiniGun          );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskLaser;",            (void *)&gam_bMaskLaser            );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskCannon;",           (void *)&gam_bMaskCannon           );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskTeleGun;",          (void *)&gam_bMaskTeleGun          );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fWeaponsFireRate;",      (void *)&gam_fWeaponsFireRate      );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fWeaponsDamage;",        (void *)&gam_fWeaponsDamage        );
	_pShell->DeclareSymbol("persistent user FLOAT gam_fProjectileSpeed;",      (void *)&gam_fProjectileSpeed      );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fGibLife;",              (void *)&gam_fGibLife              );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fEnemySpeedAdjuster;",   (void *)&gam_fEnemySpeedAdjuster   );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fPlayerDamageAdjuster;", (void *)&gam_fPlayerDamageAdjuster );
  _pShell->DeclareSymbol("persistent user INDEX gam_bUseBots;",              (void *)&gam_bUseBots              );
  _pShell->DeclareSymbol("persistent user INDEX gam_iBotsPerPlayer;",        (void *)&gam_iBotsPerPlayer        );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fBotSkill;",             (void *)&gam_fBotSkill             );
  _pShell->DeclareSymbol("persistent user INDEX gam_iPostTGPLevel;",         (void *)&gam_iPostTGPLevel         );
	_pShell->DeclareSymbol("persistent user INDEX gam_bSkipLuxor;",						 (void *)&gam_bSkipLuxor            );
	_pShell->DeclareSymbol("persistent user INDEX gam_bSkipSewers;",					 (void *)&gam_bSkipSewers           );
	_pShell->DeclareSymbol("persistent user INDEX gam_bRandomLevels;",				 (void *)&gam_bRandomLevels				  );
  _pShell->DeclareSymbol("persistent user INDEX gam_bControlFlying;",        (void *)&gam_bControlFlying        );
  _pShell->DeclareSymbol("persistent user INDEX gam_bTestSpawnerBatteries;", (void *)&gam_bTestSpawnerBatteries );
	_pShell->DeclareSymbol("persistent user INDEX gam_bUsePEHealth;",          (void *)&gam_bUsePEHealth					);
	_pShell->DeclareSymbol("persistent user INDEX gam_iFPS;",									 (void *)&gam_iFPS									);
  ...
  //_strModURL = " http://home.woh.rr.com/dwksplace";
	//_strModURL = " http://dwksplace.pro-sap.com";
  _strModURL = " http://danceswithkleer.com";
	...
	while (str != "") {
		...
		CPrintF("strTemp: %s, strTemp.Length(): %d\n", (const char *)strTemp, strTemp.Length());
		CPrintF("str: %s\n", (const char *)str);
		strTemp += "\n";
		str.ReplaceSubstr(strTemp, "");
		ctLoops++;
	}*/
}
...
static void PrintStats( CDrawPort *pdpDrawPort)
{
  ...
  if( hud_bShowClock) {
    ...
    pdpDrawPort->SetFont( _pfdDisplayFont);
    pdpDrawPort->SetTextAspect( 1.0f);
    pdpDrawPort->SetTextScaling( 0.9f*fTextScale);
    ...
    newtime = localtime(&long_time);
	  ...
	  if (newtime->tm_hour>12) {
		  newtime->tm_hour -= 12;
		} else if (newtime->tm_hour==0) {
		  newtime->tm_hour = 12;
		}

		pdpDrawPort->SetFont( &_fdNumbersFont);
		pdpDrawPort->SetTextAspect( 1.0f);
		pdpDrawPort->SetTextScaling( fTextScale*0.4f);

    // printout
    CTString strTimeHours, strTimeMinutes, strColon, strAMPMOut;
		strTimeHours.PrintF( "%2d", newtime->tm_hour);
    pdpDrawPort->PutTextR( strTimeHours,  slDPWidth*0.7f, slDPHeight*0.97f, C_WHITE|CT_OPAQUE);
		if (newtime->tm_min<10) {
			strTimeMinutes.PrintF( "0%d", newtime->tm_min);
		} else {
			strTimeMinutes.PrintF( "%2d", newtime->tm_min);
		}
    pdpDrawPort->PutText( strTimeMinutes,  slDPWidth*0.71f, slDPHeight*0.97f, C_WHITE|CT_OPAQUE);
  ...
  hud_iStats = Clamp( hud_iStats, (INDEX)0, (INDEX)2);
	...
	// display fps and stats if needed
	if (hud_iStats > 0) {
		// set display interface (proportional) font
		pdpDrawPort->SetFont( &_fdNumbersFont);
		pdpDrawPort->SetTextAspect( 1.0f);
		...
		// printout FPS number
		pdpDrawPort->PutTextR( strFPS, slDPWidth*0.843f, slDPHeight*0.97f, colFPS|245);
		...
		pdpDrawPort->SetTextScaling( 0.6f*fTextScale);
		strFPS.PrintF("FPS\n");
		pdpDrawPort->PutTextR( strFPS, slDPWidth*0.87f, slDPHeight*0.979f, colFPS|245);

		// draw filter for mod effect
	//  pdpDrawPort->Fill( 0,0, slDPWidth, slDPHeight, C_lGREEN|5);

		// if in extensive stats mode display extensive statistics
...
static void MakeSplitDrawports(enum CGame::SplitScreenCfg ssc, INDEX iCount, CDrawPort *pdp)
{
  ...
  if (ssc!=CGame::SSC_OBSERVER && iCount>=1) {
    ...
    #define FULL (4/16.0)
    if (iCount==1) {
			adpDrawPorts[iFirstObserver] = CDrawPort( pdp, 1.0-FREE-FULL, FREE, FULL, FULL);
      apdpDrawPorts[iFirstObserver] = &adpDrawPorts[iFirstObserver];
    } else if (iCount==2) {
			adpDrawPorts[iFirstObserver] = CDrawPort( pdp, 1.0-FREE-FULL, FREE+0*(FULL+FREE), FULL, FULL);
      apdpDrawPorts[iFirstObserver] = &adpDrawPorts[iFirstObserver];
    ...
    && gm_CurrentSplitScreenCfg!=SSC_DEDICATED )
  {
    ...
    {for (INDEX i=0; i<16; i++) {
      ...
      if (pen!=NULL && !_pNetwork->IsPlayerLocal(pen)) {
		...
		//CPrintF("ctLocals: %d, ctNonlocals: %d\n", ctLocals, ctNonlocals);
    ...
    if ((ulFlags&GRV_SHOWEXTRAS) && dpMsg.Lock())
    {
      ...
      PrintStats( &dpMsg);
			//INDEX iPlayer = ((CPlayerEntity*)this)->GetMyPlayerIndex();
			//CPrintF("iPlayer: %d, GetPlayersCount(): %d\n", iPlayer, GetPlayersCount());
...
/*static CTextureObject _toBcgFace;
...
void TiledTextureDK( PIXaabbox2D &_boxScreen, MEX2D &vScreen, MEXaabbox2D &boxTexture, int iPixOffsetW, int iPixOffsetH)
{
  PIX pixW = _boxScreen.Size()(1/2);//-iPixOffsetW;
  PIX pixH = _boxScreen.Size()(2)/2;//-iPixOffsetH;
	//CPrintF("pixW: %d, pixH: %d\n", pixW, pixH);
  boxTexture = MEXaabbox2D(MEX2D(0, 0), MEX2D(pixW, pixH));
	boxTexture += MEX2D(0, 4);
	//MEX mexW = vScreen(1);
	//MEX mexH = vScreen(2);
	//CPrintF("mexW: %d, mexH: %d\n", mexW, mexH);
  boxTexture += vScreen;
}
...
void CGame::LCDInit(void)
{
  try {
    ...
    //_toBcgFace.SetData_t(CTFILENAME("TexturesMP\\General\\Face.tex"));
    ...
    //((CTextureData*)_toBcgFace.GetData())->Force(TEX_CONSTANT);
...
{
	/*if (!_bPopup) {
	//	_pdp_SE->PutTexture(&_toBcgFace, _boxScreen_SE, C_GRAY|255);
		// menu background
		MEXaabbox2D boxBcgFace;
		TiledTextureDK(_boxScreen_SE, 0.31f, MEX2D(_toBcgFace.GetWidth(), _toBcgFace.GetHeight()), boxBcgFace);
		_pdp_SE->PutTexture(&_toBcgFace, _boxScreen_SE, boxBcgFace, C_dGRAY|100);
	}*/

	// menu background
	MEXaabbox2D boxBcgBackdrop;
	TiledTextureSE(_boxScreen_SE, 1.0f, MEX2D(_toBackdrop.GetWidth(), _toBackdrop.GetHeight()), boxBcgBackdrop);
	//TiledTextureDK(_boxScreen_SE, MEX2D(4, 8), boxBcgBackdrop, 2, 2);
	//_pdp_SE->PutTexture(&_toBackdrop, _boxScreen_SE, boxBcgBackdrop, C_WHITE|150);
  ...
  if (dmCurrent.IsFullScreen()) {
    ...
    if (!_pInput->IsInputEnabled()) {
      //while (ShowCursor(TRUE) < 0);
			while (ShowCursor(FALSE) >= 0);
			//CPrintF("_pInput->inp_rectOldClip: %d\n", _pInput->inp_rectOldClip);
    } /*else {
			while (ShowCursor(TRUE) < 0);
			return;
		}*/
...
COLOR CGame::LCDGetColor(COLOR colDefault, const char *strName)
{
 if (!strcmp(strName, "thumbnail border")) {
    ...
    colDefault = C_WHITE|255;
  } else if (!strcmp(strName, "editing")) { // font in editing box i.e. player name
    colDefault = C_BLACK|255;
  } else if (!strcmp(strName, "hilited")) {
    colDefault = C_WHITE|255;
  } else if (!strcmp(strName, "hilited rectangle")) {
    colDefault = C_BLUE|255;
  } else if (!strcmp(strName, "edit fill")) { // background of editing box
    colDefault = C_lGRAY|255;
  } else if (!strcmp(strName, "editing cursor")) {
    colDefault = C_WHITE|255;
    ...
    colDefault = C_WHITE|255;
  } else if (!strcmp(strName, "slider box")) { // mouse sensitivity
    colDefault = C_GRAY|255;
  //} else if (!strcmp(strName, "slider editing")) { // mouse sensitivity font?
    //colDefault = C_BLACK|255;
  } else if (!strcmp(strName, "file info")) {
    colDefault = C_WHITE|255;
  } else if (!strcmp(strName, "display mode")) {
...
{
	if (!_bPopup && !IsScriptSoundPlaying(14)) { 
		PlayScriptSound(14, CTFILENAME("MusicMP\\CutScenes\\LastScene.ogg"), 3.0f, 1.0f, FALSE);
	}
}
