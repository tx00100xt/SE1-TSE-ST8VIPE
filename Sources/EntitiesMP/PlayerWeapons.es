...
//#include "Models/Weapons/Mini1/Mini1GunItem.h"
//#include "Models/Weapons/Mini1/Mini1Gun.h"
//#include "Models/Weapons/Mini1/Body.h"
//#include "Models/Weapons/Mini1/Barrels.h"
//#include "Models/Weapons/Mini1/Engine.h"

#include "Models/Weapons/Bomb/Bomb.h"
#include "Models/Weapons/Bomb/BombSmall.h"

#include "Models/Weapons/Mini2/Mini2Gun.h"
#include "Models/Weapons/Mini2/Mini2GunItem.h"
#include "Models/Weapons/Mini2/Body.h"
#include "Models/Weapons/Mini2/Barrels01.h"
#include "Models/Weapons/Mini2/Barrels02.h"
#include "Models/Weapons/Mini2/Engine.h"
...
#include "Models/Weapons/Mini2/Barrels02.h"
#include "Models/Weapons/Mini2/Engine.h"
#include "Models/TeleporterGun/GunHand.h"
...
//#include "EntitiesMP/GhostBusterRay.h"
extern INDEX hud_bShowWeapon;
extern INDEX hud_iCrosshairColor;
...
uses "EntitiesMP/LightningRay";
uses "EntitiesMP/TransProjectile";
uses "EntitiesMP/CannonBall";
uses "EntitiesMP/Rail";
...
#define WEAPONS_ALLAVAILABLEMASK 0x3FFF
  ...
  #define WEAPONS_DISABLEDMASKCOLT (\
    (1<<(WEAPON_COLT           -1))|\
    (1<<(WEAPON_DOUBLECOLT     -1)))

  #define WEAPONS_DISABLEDMASKSINGLESHOTGUN (\
    (1<<(WEAPON_SINGLESHOTGUN  -1)))
  
  #define WEAPONS_DISABLEDMASKSNIPER (\
    (1<<(WEAPON_SNIPER  -1)))

  #define WEAPONS_DISABLEDMASKFLAMER (\
    (1<<(WEAPON_FLAMER        -1)))

  #define WEAPONS_DISABLEDMASKTOMMYGUN (\
    (1<<(WEAPON_TOMMYGUN  -1)))

  #define WEAPONS_DISABLEDMASKMINIGUN (\
    (1<<(WEAPON_MINIGUN  -1)))

  #define WEAPONS_DISABLEDMASKROCKETLAUNCHER (\
    (1<<(WEAPON_ROCKETLAUNCHER  -1)))

  #define WEAPONS_DISABLEDMASKGRENADELAUNCHER (\
    (1<<(WEAPON_GRENADELAUNCHER  -1)))

  #define WEAPONS_DISABLEDMASKLASER (\
    (1<<(WEAPON_LASER  -1)))

  #define WEAPONS_DISABLEDMASKIRONCANNON (\
    (1<<(WEAPON_IRONCANNON  -1)))

  #define WEAPONS_DISABLEDMASKCHAINSAW (\
    (1<<(WEAPON_CHAINSAW       -1)))

  #define WEAPONS_DISABLEDMASKDOUBLESHOTGUN (\
    (1<<(WEAPON_DOUBLESHOTGUN  -1)))
...
static INDEX iSniperDiscreteZoomLevels = 9;
static FLOAT afSniperZoom[] =
{
//      90.0f,1.0f, 53.1f, 2.0f, 28.0f,4.0f, 14.2f,6.0f, 
       //7.2f,8.0f, 3.56f,10.0f ,1.8f,12.0f
      90.0f,1.0f, 75.0f, 2.0f, 53.1f, 3.0f, 
		  40.0f, 4.0f, 28.0f,5.0f, 21.0f, 6.0f, 
		  14.2f,7.0f, 10.0f, 8.0f, 7.2f, 9.0f, 
};
...
static CTextureObject _toSniperCrosshair;
...
void CPlayerWeapons_Precache(ULONG ulAvailable)
{
  ...
  if ( ulAvailable&(1<<(WEAPON_KNIFE-1)) ) {
    ...
    pdec->PrecacheModel(MODEL_KNIFE_PROJECTILE      );
		pdec->PrecacheTexture(TEXTURE_KNIFE_PROJECTILE  );
		pdec->PrecacheTexture(TEXTURE_KNIFE_IMPACT      );
  ...
  if ( ulAvailable&(1<<(WEAPON_COLT-1)) ) {
    ...
    pdec->PrecacheModel(MODEL_COLTMAIN_L            );
    ...
    pdec->PrecacheSound(SOUND_COLT_FIRE2            );
  }
  ...
  if ( ulAvailable&(1<<(WEAPON_DOUBLESHOTGUN-1)) ) {
    pdec->PrecacheModel(MODEL_TP_HAND        ); 
    pdec->PrecacheModel(MODEL_TP_GUN         ); 
    pdec->PrecacheClass(CLASS_TRANSPROJECTILE );
    ...
    pdec->PrecacheModel(MODEL_BOMB_SMALL      );
		pdec->PrecacheModel(MODEL_BOMB_BIG        );
		pdec->PrecacheModel(MODEL_BOMB_ITEM       );
    pdec->PrecacheTexture(TEXTURE_BOMB        ); 
    pdec->PrecacheClass(CLASS_PROJECTILE, PRT_BOMB);
    pdec->PrecacheSound(SOUND_MINIGUN_FIRE			);
//    pdec->PrecacheSound(SOUND_BOMB_THROW			); 
  }
/*
    if ( ulAvailable&(1<<(WEAPON_TOMMYGUN-1)) ) {
    pdec->PrecacheModel(MODEL_TOMMYGUN              );
    pdec->PrecacheModel(MODEL_TG_BODY               );
    pdec->PrecacheModel(MODEL_TG_ENGINE             );
    pdec->PrecacheModel(MODEL_TG_BARRELS            );
    pdec->PrecacheTexture(TEXTURE_TG_BODY           ); 
    pdec->PrecacheTexture(TEXTURE_TG_BARRELS        );
    pdec->PrecacheSound(SOUND_MINIGUN_FIRE			);
...
*/
  if ( ulAvailable&(1<<(WEAPON_SNIPER-1)) ) {
    ...
    pdec->PrecacheClass(CLASS_RAIL            );
    //pdec->PrecacheModel(MODEL_SNIPER_ITEM     );
  }

  if ( ulAvailable&(1<<(WEAPON_MINIGUN-1)) ) {
    pdec->PrecacheModel(MODEL_MINIGUN          ); 
    pdec->PrecacheModel(MODEL_MG_BARRELS01     );  
		pdec->PrecacheModel(MODEL_MG_BARRELS02     );
    ...
    pdec->PrecacheSound(SOUND_MINIGUN_FIRE2    );     
    pdec->PrecacheSound(SOUND_MINIGUN_ROTATE   );     
    pdec->PrecacheSound(SOUND_MINIGUN_SPINUP   );     
    pdec->PrecacheSound(SOUND_MINIGUN_SPINDOWN );     
    pdec->PrecacheSound(SOUND_MINIGUN_CLICK    );
    pdec->PrecacheClass(CLASS_TRACER);
  }
  ...
  if ( ulAvailable&(1<<(WEAPON_CHAINSAW-1))) {
    ...
    pdec->PrecacheModel(MODEL_CHAINSAW_ITEM);
  }
  ...
  if ( ulAvailable&(1<<(WEAPON_FLAMER-1)) ) {
    ...
    pdec->PrecacheClass(CLASS_PROJECTILE, PRT_FLAMER_PROJECTILE);
    pdec->PrecacheModel(MODEL_FLAMER_ITEM );
  }
  ...
  if ( ulAvailable&(1<<(WEAPON_LASER-1)) ) {
    ...
    pdec->PrecacheClass(CLASS_PROJECTILE, PRT_LASER_BOMB);
  }
...
static FLOAT afTommygunShellPos[3] = { 0.0f, 0.05f, -0.21f};
static FLOAT afTommygunShellPosLeft[3] = { -0.45f, 0.05f, -0.24f};

static FLOAT afTommygunShellPos3rd[3] = { 0.125f, -0.1f, -0.87f};
static FLOAT afTommygunShellPosLeft3rd[3] = { -0.45f, -0.1f, -0.9f};
...
properties:
 ...
 13 BOOL  m_bSecondaryFire = FALSE,      // secondary fire weapon
 ...
 56 INDEX m_iNukeBalls      = 0,
 57 INDEX m_iMaxNukeBalls   = MAX_NUKEBALLS,
...
234 FLOAT m_fSniperMinFOV = 7.2f,
...
1001 CEntityPointer m_penRocketLauncherHoming,  // target for rocket launcher
1002 BOOL m_bMaskFlamerDoubleShotgun = TRUE, // remove the crap
1003 INDEX m_iCannonThang = 0,  // cannonball color counter
1004 CEntityPointer m_penFlamerProjectile,  // target for flamer projectile
1005 FLOAT m_fRate = 1.0f,
1006 FLOAT m_fDamageAmount = 1.0f,
1007 CEntityPointer m_penTP,
1008 INDEX m_iTracerCount = 0,
1009 FLOAT3D m_vRayHitCoords = FLOAT3D(0,0,0), // coordinates where ray hit
...
components:
  ...
  5 class   CLASS_LIGHTNINGRAY      "Classes\\LightningRay.ecl",
  ...
  9 class   CLASS_TRANSPROJECTILE   "Classes\\TransProjectile.ecl",
  11 class  CLASS_RAIL              "Classes\\Rail.ecl",
  12 class  CLASS_TRACER            "Classes\\Tracer.ecl",
...
// ************** KNIFE **************
 ...
 190 model   MODEL_KNIFE_PROJECTILE      "Models\\WeaponsHD\\Knife\\Projectile\\KnifeProjectile.mdl",
 191 texture TEXTURE_KNIFE_PROJECTILE    "Models\\WeaponsHD\\Knife\\Projectile\\KnifeProjectile.tex",
 192 texture TEXTURE_KNIFE_IMPACT        "Models\\WeaponsHD\\Knife\\Projectile\\KnifeProjectileHit2.tex",
 193 sound   SOUND_KNIFE_THROW           "Models\\WeaponsHD\\Knife\\Sounds\\Throw.wav",
...
// ************** COLT **************
 ...
 39 sound   SOUND_COLT_FIRE2            "Models\\Weapons\\Colt\\Sounds\\Fire2.wav",
 47 model   MODEL_COLTMAIN_L            "Models\\Weapons\\Colt\\ColtMainL.mdl",
...
// ************** DOUBLE SHOTGUN **************
 51 model   MODEL_TP_HAND               "Models\\TeleporterGun\\GunHand.mdl",
 52 model   MODEL_TP_GUN                "Models\\TeleporterGun\\Gun.mdl",
 53 texture TEXTURE_TP                  "Models\\TeleporterGun\\Gun.tex",
...
// ************** TOMMYGUN **************
 70 model   MODEL_BOMB                  "Models\\Weapons\\Bomb\\Bomb.mdl",
 71 model   MODEL_BOMB_SMALL            "Models\\Weapons\\Bomb\\BombSmall.mdl",
 72 model   MODEL_BOMB_ITEM             "Models\\Weapons\\Bomb\\BombItem.mdl",
 73 model   MODEL_BOMB_BIG              "Models\\Weapons\\Bomb\\BombBig.mdl",
 76 texture TEXTURE_BOMB                "Models\\Weapons\\Bomb\\Bomb.tex",
// 77 sound   SOUND_BOMB_THROW            "Models\\Weapons\\Bomb\\Sounds\\BombThrow.wav",
 /* 70 model   MODEL_TOMMYGUN              "Models\\Weapons\\Mini1\\Mini1Gun.mdl",
 71 model   MODEL_TG_BODY               "Models\\Weapons\\Mini1\\Body.mdl",
 72 model   MODEL_TG_BARRELS            "Models\\Weapons\\Mini1\\Barrels.mdl",
 73 model   MODEL_TG_ENGINE             "Models\\Weapons\\Mini1\\Engine.mdl",
 75 texture TEXTURE_TG_BODY             "Models\\Weapons\\Mini1\\Body.tex",
 76 texture TEXTURE_TG_BARRELS          "Models\\Weapons\\Mini1\\Barrels.tex",*/
 77 sound   SOUND_MINIGUN_FIRE          "Models\\Weapons\\MiniGun\\Sounds\\Fire.wav",

// ************** MINIGUN **************
 300 model   MODEL_MINIGUN                "Models\\Weapons\\Mini2\\Mini2Gun.mdl",
 301 model   MODEL_MG_BODY                "Models\\Weapons\\Mini2\\Body.mdl",
 302 texture TEXTURE_MG_BODY              "Models\\Weapons\\Mini2\\Body.tex",
 303 model   MODEL_MG_BARRELS01           "Models\\Weapons\\Mini2\\Barrels01.mdl",
 304 model   MODEL_MG_BARRELS02           "Models\\Weapons\\Mini2\\Barrels02.mdl",
 305 texture TEXTURE_MG_BARRELS           "Models\\Weapons\\Mini2\\Barrels.tex",
 306 model   MODEL_MG_ENGINE              "Models\\Weapons\\Mini2\\Engine.mdl",
 307 sound   SOUND_MINIGUN_FIRE2          "Models\\Weapons\\Mini2\\Sounds\\Fire.wav",
 308 sound   SOUND_MINIGUN_ROTATE         "Models\\Weapons\\MiniGun\\Sounds\\Rotate.wav",
 309 sound   SOUND_MINIGUN_SPINUP         "Models\\Weapons\\MiniGun\\Sounds\\RotateUp.wav",
 310 sound   SOUND_MINIGUN_SPINDOWN       "Models\\Weapons\\MiniGun\\Sounds\\RotateDown.wav",
 311 sound   SOUND_MINIGUN_CLICK          "Models\\Weapons\\MiniGun\\Sounds\\Click.wav",
...
//114 model   MODEL_SNIPER_ITEM           "ModelsMP\\Weapons\\Sniper\\SniperItem.mdl",
...
300 model   MODEL_FLAMER_ITEM           "ModelsMP\\Weapons\\Flamer\\FlamerItem.mdl",
...
163 model   MODEL_CHAINSAW_ITEM         "ModelsMP\\Weapons\\Chainsaw\\ChainsawItem.mdl",
...
functions:
 ...
 // add to prediction any entities that this entity depends on
  ...
  {
	  return m_bSecondaryFire && !m_bChangeWeapon;
  }
  ...
  void UpdateTargetingInfo(void)
  {
    ...
    m_vRayHitCoords = crRay.cr_vHit; // coord fix
    ...
    if( m_penRayHit!=NULL)
    {
      ...
      if( pen->GetFlags()&ENF_ALIVE)
      {
        ...
        if( IsOfClass( pen, "PlayerBot")) {
          ...
          // keep player name, mana and health for eventual printout or coloring
          m_strLastTarget.PrintF( "%s", (const char *) ((CPlayerBot*)pen)->GetName());
          if( hud_bShowPlayerName) { m_tmLastTarget = tmNow+1.5f; }
        }
        ...
        if( IsDerivedFromClass( pen, "Enemy Base")) {
          ...
          /*CTString strInfo = "";
          strInfo.PrintF( " (%d)", ((CEnemyBase*)pen)->);
      ...
      else
      {
        ...
        if( IsOfClass( pen, "Switch") && m_fRayHitDistance<6.0f) {
          CSwitch &enSwitch = (CSwitch&)*pen;
          // if switch is useable
          if( enSwitch.m_bUseable) {
            SendToTarget(pen, EET_TRIGGER, m_penPlayer);
            // show switch message
            if( enSwitch.m_strMessage!="") { 
							m_strLastTarget = enSwitch.m_strMessage; 
						} else { 
							m_strLastTarget = TRANS("U s e d !"); 
						}
            m_tmLastTarget = tmNow+3.0f;
          }
        }
        // if analyzable
  ...
  void RenderCrosshair( CProjection3D &prProjection, CDrawPort *pdp, CPlacement3D &plViewSource)
  {
    ...
    _toSniperCrosshair.SetData_t( CTFILENAME("Textures\\Interface\\Crosshairs\\SniperCrosshair.tex"));
    ...
    COLOR colCrosshair = C_WHITE;
		...
		if        (hud_iCrosshairColor==1) { colCrosshair = C_RED;
		} else if (hud_iCrosshairColor==2) { colCrosshair = C_BLUE;
		} else if (hud_iCrosshairColor==3) { colCrosshair = C_GREEN;
		} else if (hud_iCrosshairColor==4) { colCrosshair = C_YELLOW;
		} else if (hud_iCrosshairColor==5) { colCrosshair = C_MAGENTA;
		} else if (hud_iCrosshairColor==6) { colCrosshair = C_GRAY;
		} else                               { colCrosshair = C_WHITE; }
    ...
    const FLOAT3D vRayHitCoords = m_vRayHitCoords; // crosshair fix dorked the coord printout
    // if hit anything
    if( m_penRayHit!=NULL) {
      if( hud_bCrosshairColoring && m_fEnemyHealth>0) {
        if( m_fEnemyHealth<0.25f) {
					if (hud_iCrosshairColor != 1) { colCrosshair = C_RED;  } else { colCrosshair = C_WHITE; }
				} else if( m_fEnemyHealth<0.60f){
					if (hud_iCrosshairColor != 4) { colCrosshair = C_YELLOW; } else { colCrosshair = C_WHITE; }
				} else{ 
					if (hud_iCrosshairColor != 3) { colCrosshair = C_GREEN; } else { colCrosshair = C_WHITE; }
 				}
      }
    }
    ...
    if( iCrossHair>0) {
			if (m_bSniping) {
				...
				const FLOAT fSize    = 16 *hud_fCrosshairScale;
				// draw crosshair
				...
				pdp->InitTexture( &_toSniperCrosshair);
...
//      pdp->PutTextC( m_strLastTarget, slDPWidth*0.5f, slDPHeight*0.75f, SE_COL_BLUE_NEUTRAL|ulA);
      pdp->PutTextC( m_strLastTarget, slDPWidth*0.5f, slDPHeight*0.75f, C_WHITE|ulA);
    }
    ...
    if( hud_bShowCoords) { 
      ...
      const FLOAT fMax = Max( Max( vRayHitCoords(1), vRayHitCoords(2)), vRayHitCoords(3));
      const FLOAT fMin = Min( Min( vRayHitCoords(1), vRayHitCoords(2)), vRayHitCoords(3));
      if( fMax<+100000 && fMin>-100000) {
        strCoords.PrintF( "%.0f,%.0f,%.0f", vRayHitCoords(1), vRayHitCoords(2), vRayHitCoords(3));
 ...
 ************************************************************/
    ...
    if (pen->m_iFlare==FLARE_ADD) {
      ...
      switch(m_iCurrentWeapon) {
        ...
        case WEAPON_DOUBLECOLT: 
          ShowFlare(m_moWeapon, COLT_ATTACHMENT_COLT, COLTMAIN_ATTACHMENT_FLARE, 0.75f);
          ShowFlare(m_moWeaponSecond, COLT_ATTACHMENT_COLT, COLTMAIN_ATTACHMENT_FLARE, 0.75f);
          break;
        ...
        case WEAPON_TOMMYGUN:
          ShowFlare(m_moWeapon, MINI1GUN_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE, 1.25f);
          break;*/
        case WEAPON_SNIPER:
          ShowFlare(m_moWeapon, SNIPER_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE, 0.5f);
          break;
        case WEAPON_MINIGUN:
          ShowFlare(m_moWeapon, MINI2GUN_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE01, 1.25f);
          ShowFlare(m_moWeapon, MINI2GUN_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE02, 1.25f);
          break;
      }
    // remove
      ...
      switch(m_iCurrentWeapon) {
        ...
        /*case WEAPON_DOUBLESHOTGUN:
          ...
          break;
        case WEAPON_TOMMYGUN:
          HideFlare(m_moWeapon, MINI1GUN_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE);
          break;*/
        case WEAPON_SNIPER:
          HideFlare(m_moWeapon, SNIPER_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE);
          break;
        case WEAPON_MINIGUN:
          HideFlare(m_moWeapon, MINI2GUN_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE01);
          HideFlare(m_moWeapon, MINI2GUN_ATTACHMENT_BODY, BODY_ATTACHMENT_FLARE02);
          break;
      }
    } else {
      ASSERT(FALSE);
  ...
  void SetCurrentWeaponModel(void) {
    ...
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_DOUBLECOLT: {
        ...
        AddAttachmentToModel(this, m_moWeaponSecond, COLT_ATTACHMENT_BULLETS, MODEL_COLTBULLETS, TEXTURE_COLTBULLETS, 0, 0, 0);
        AddAttachmentToModel(this, m_moWeaponSecond, COLT_ATTACHMENT_COCK, MODEL_COLTCOCK, TEXTURE_COLTCOCK, 0, 0, 0);
        AddAttachmentToModel(this, m_moWeaponSecond, COLT_ATTACHMENT_COLT, MODEL_COLTMAIN_L, TEXTURE_COLTMAIN, 0, TEX_SPEC_MEDIUM, 0);
      ...
      case WEAPON_COLT: {
        ...
        AddAttachmentToModel(this, m_moWeapon, COLT_ATTACHMENT_BULLETS, MODEL_COLTBULLETS, TEXTURE_COLTBULLETS, 0, 0, 0);
        AddAttachmentToModel(this, m_moWeapon, COLT_ATTACHMENT_COCK, MODEL_COLTCOCK, TEXTURE_COLTCOCK, 0, 0, 0);
        AddAttachmentToModel(this, m_moWeapon, COLT_ATTACHMENT_COLT, MODEL_COLTMAIN, TEXTURE_COLTMAIN, 0, TEX_SPEC_MEDIUM, 0);
      ...
      case WEAPON_SINGLESHOTGUN: {
        ...
        break; }
      case WEAPON_DOUBLESHOTGUN: {
        SetComponents(this, m_moWeapon, MODEL_TP_HAND, TEXTURE_HAND, 0, 0, 0);
        AddAttachmentToModel(this, m_moWeapon, GUNHAND_ATTACHMENT_GUN, MODEL_TP_GUN, TEXTURE_TP, TEX_REFL_BWRIPLES01, TEX_SPEC_MEDIUM, 0);
        m_moWeapon.PlayAnim(GUNHAND_ANIM_WAIT1, 0);
        ...
        break; }
      case WEAPON_TOMMYGUN: {
        SetComponents(this, m_moWeapon, MODEL_BOMB, TEXTURE_HAND, 0, 0, 0);
        AddAttachmentToModel(this, m_moWeapon, BOMB_ATTACHMENT_BOMB, MODEL_BOMB_SMALL, TEXTURE_BOMB, TEX_REFL_DARKMETAL, TEX_SPEC_MEDIUM, 0);
        CModelObject &mo = m_moWeapon.GetAttachmentModel(BOMB_ATTACHMENT_BOMB)->amo_moModelObject;
        break; }    
			/*case WEAPON_TOMMYGUN: {
        SetComponents(this, m_moWeapon, MODEL_TOMMYGUN, TEXTURE_HAND, 0, 0, 0);
        AddAttachmentToModel(this, m_moWeapon, MINI1GUN_ATTACHMENT_BODY, MODEL_TG_BODY, TEXTURE_TG_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        AddAttachmentToModel(this, m_moWeapon, MINI1GUN_ATTACHMENT_BARRELS, MODEL_TG_BARRELS, TEXTURE_TG_BARRELS, 0, TEX_SPEC_MEDIUM, 0);
        AddAttachmentToModel(this, m_moWeapon, MINI1GUN_ATTACHMENT_ENGINE, MODEL_TG_ENGINE, TEXTURE_TG_BARRELS, 0, TEX_SPEC_MEDIUM, 0);
        CModelObject &mo = m_moWeapon.GetAttachmentModel(MINI1GUN_ATTACHMENT_BODY)->amo_moModelObject;
      ...
      case WEAPON_MINIGUN: {
        ...
        AddAttachmentToModel(this, m_moWeapon, MINI2GUN_ATTACHMENT_BODY, MODEL_MG_BODY, TEXTURE_MG_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
				AddAttachmentToModel(this, m_moWeapon, MINI2GUN_ATTACHMENT_BARRELS01, MODEL_MG_BARRELS01, TEXTURE_MG_BARRELS, TEX_REFL_LIGHTBLUEMETAL01, TEX_SPEC_MEDIUM, 0);
        AddAttachmentToModel(this, m_moWeapon, MINI2GUN_ATTACHMENT_BARRELS02, MODEL_MG_BARRELS02, TEXTURE_MG_BARRELS, TEX_REFL_LIGHTBLUEMETAL01, TEX_SPEC_MEDIUM, 0);
				AddAttachmentToModel(this, m_moWeapon, MINI2GUN_ATTACHMENT_ENGINE, MODEL_MG_ENGINE, TEXTURE_MG_BARRELS, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
        CModelObject &mo = m_moWeapon.GetAttachmentModel(MINI2GUN_ATTACHMENT_BODY)->amo_moModelObject;
        AddAttachmentToModel(this, mo, BODY_ATTACHMENT_FLARE01, MODEL_FLARE01, TEXTURE_FLARE01, 0.13, 0, 0);
        AddAttachmentToModel(this, mo, BODY_ATTACHMENT_FLARE02, MODEL_FLARE01, TEXTURE_FLARE01, -0.13, 0, 0);
        break; }
  ...
  void RotateMinigun(void) {
    if(m_iCurrentWeapon==WEAPON_MINIGUN) { 
      ...
      //CPrintF( "Player aAngle: %.3f\n", aAngle);
			// rotate minigun barrels
			CAttachmentModelObject *amo1 = m_moWeapon.GetAttachmentModel(MINI2GUN_ATTACHMENT_BARRELS01);
			amo1->amo_plRelative.pl_OrientationAngle(3) = -aAngle;
			CAttachmentModelObject *amo2 = m_moWeapon.GetAttachmentModel(MINI2GUN_ATTACHMENT_BARRELS02);
			amo2->amo_plRelative.pl_OrientationAngle(3) = aAngle;
    }
		/*else if(m_iCurrentWeapon==WEAPON_TOMMYGUN) {
			ANGLE aAngle = Lerp(m_aMiniGunLast, m_aMiniGun, _pTimer->GetLerpFactor());
			// rotate tommygun barrels
			CAttachmentModelObject *amo = m_moWeapon.GetAttachmentModel(MINI1GUN_ATTACHMENT_BARRELS);
  ...
  BOOL CutWithKnife(FLOAT fX, FLOAT fY, FLOAT fRange, FLOAT fWide, FLOAT fThickness, FLOAT fDamage) 
  {
    ...
    if (penClosest!=NULL) {
      ...
      InflictRangeDamage(m_penPlayer, DMT_CLOSERANGE, 100.0f, plKnife.pl_PositionVector, 4.0f, 6.0f);
      return TRUE;
    }
    return FALSE;
  };
  ...
  void FireSniperBullet(FLOAT fX, FLOAT fY, FLOAT fRange, FLOAT fDamage, FLOAT fImprecission) {
    ...
    ((CBullet&)*penBullet).m_fBulletSize = 0.1f;
		//((CBullet&)*penBullet).m_EdtDamage = DMT_ACID;
    // launch bullet
    ((CBullet&)*penBullet).LaunchBullet(TRUE, FALSE, TRUE, FALSE);    
  ...
  // prepare explosive bullet
  void PrepareExplosiveBullet(FLOAT fX, FLOAT fY, FLOAT fDamage, FLOAT fImprecission) {
    // bullet start position
    CalcWeaponPositionImprecise(FLOAT3D(fX, fY, 0), plBullet, TRUE, fImprecission);
    // create bullet
    penBullet = CreateEntity(plBullet, CLASS_BULLET);
  ...
  // fire one explosive bullet
  void FireExplosiveBullet(FLOAT fX, FLOAT fY, FLOAT fRange, FLOAT fDamage, FLOAT fImprecission) {
    PrepareExplosiveBullet(fX, fY, fDamage, fImprecission);
    ((CBullet&)*penBullet).CalcTarget(fRange);
    ((CBullet&)*penBullet).m_fBulletSize = 0.2f;
    // launch bullet
    ((CBullet&)*penBullet).LaunchBullet(TRUE, FALSE, FALSE, TRUE);
    
    if (((CBullet&)*penBullet).m_vHitPoint != FLOAT3D(0.0f, 0.0f, 0.0f)) {
      m_vBulletTarget = ((CBullet&)*penBullet).m_vHitPoint;
    } else if (TRUE) {
      m_vBulletTarget = m_vBulletSource + FLOAT3D(0.0f, 0.0f, -200.0f)*((CBullet&)*penBullet).GetRotationMatrix();
  ...
  void FireOneBullet(FLOAT fX, FLOAT fY, FLOAT fRange, FLOAT fDamage) {
    ...
    ((CBullet&)*penBullet).CalcTarget(fRange);
    ((CBullet&)*penBullet).m_fBulletSize = 0.1f;
    ((CBullet&)*penBullet).CalcJitterTarget(0.01f);
		//((CBullet&)*penBullet).m_EdtDamage = DMT_ACID;
    // launch bullet
    ((CBullet&)*penBullet).LaunchBullet(TRUE, FALSE, TRUE, FALSE);
    ((CBullet&)*penBullet).DestroyBullet();
  ...
  void FireBullets(FLOAT fX, FLOAT fY, FLOAT fRange, FLOAT fDamage, INDEX iBullets,
    ...
    PrepareBullet(fX, fY, fDamage);
    ((CBullet&)*penBullet).CalcTarget(fRange);
    ((CBullet&)*penBullet).m_fBulletSize = 0.3f;
		//((CBullet&)*penBullet).m_EdtDamage = DMT_ACID;
    // launch slugs
    INDEX iSlug;
    for (iSlug=0; iSlug<iBullets; iSlug++) {
      ...
      ((CBullet&)*penBullet).CalcJitterTargetFixed(
        afPositions[iSlug*2+0]*fRange*(fStretch+fJitter), 
				afPositions[iSlug*2+1]*fRange*(fStretch+fJitter),
        fJitter*fRange*fStretch);
      ((CBullet&)*penBullet).LaunchBullet(iSlug<2, FALSE, TRUE, FALSE);
    }
		((CBullet&)*penBullet).DestroyBullet();
  };
  ...
  {
    fJitter*=fRange;  // jitter relative to range 7.5f
    PrepareBullet(fX, fY, fDamage);
    ((CBullet&)*penBullet).CalcTarget(fRange);
  ...
  void FireGrenade(void) {
    ...
    eLaunch.fSpeed = 120.0f;
    penGrenade->Initialize(eLaunch);
  };

	void FireGrenadeBomb(void) {
    // grenade start position
    CPlacement3D plGrenadeBomb;
    CalcWeaponPosition(
      FLOAT3D(wpn_fFX[WEAPON_GRENADELAUNCHER],wpn_fFY[WEAPON_GRENADELAUNCHER], 0), 
      plGrenadeBomb, TRUE);
    // create grenade
    CEntityPointer penGrenadeBomb = CreateEntity(plGrenadeBomb, CLASS_PROJECTILE);
    // init and launch grenade
    ELaunchProjectile eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.prtType = PRT_GRENADE_BOMB;
    eLaunch.fSpeed = 100.0f;
    penGrenadeBomb->Initialize(eLaunch);
  };
  ...
  void FireHomingRocket(void) {
	...
	// secondary fire flamer
  void FireFlamerProjectile(void) {
    // rocket start position
    CPlacement3D plFlamerProjectile;
    CalcWeaponPosition(FLOAT3D(wpn_fFX[WEAPON_FLAMER],wpn_fFY[WEAPON_FLAMER], -0.15f), plFlamerProjectile, TRUE);
    // create rocket
    CEntityPointer penFlamerProjectile= CreateEntity(plFlamerProjectile, CLASS_PROJECTILE);
    // init and launch flamer projectile
    ELaunchProjectile eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.prtType = PRT_FLAMER_PROJECTILE;
    penFlamerProjectile->Initialize(eLaunch);
	};
  ...
  void ThrowKnife(void) {
    // knife start position
    CPlacement3D plKnife;
    CalcWeaponPosition( FLOAT3D(0,0.5f,0.25f), plKnife, TRUE);
    // create knife
    CEntityPointer penKnife= CreateEntity(plKnife, CLASS_PROJECTILE);
    // init and throw knife
    ELaunchProjectile eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.prtType = PRT_KNIFE_PROJECTILE;
  ...
  // fire laser ray
  void FireLaserRail(void) {
    // laser start position
    CPlacement3D plLaserRail;
    FLOAT fFX = wpn_fFX[WEAPON_LASER];  // get laser center position
    FLOAT fFY = wpn_fFY[WEAPON_LASER];
    FLOAT fLUX = 0.0f;
    FLOAT fRUX = 0.4f;
    ...
    switch(m_iLaserBarrel) {
      case 0:   // barrel lu (*o-oo)
        CalcWeaponPosition(FLOAT3D(fFX+fLUX, fFY+fLUY, 0), plLaserRail, TRUE);
        break;
      case 1:   // barrel ld (oo-*o)
        CalcWeaponPosition(FLOAT3D(fFX+fLDX, fFY+fLDY, 0), plLaserRail, TRUE);
        break;
      case 2:   // barrel ru (o*-oo)
        CalcWeaponPosition(FLOAT3D(fFX+fRUX, fFY+fRUY, 0), plLaserRail, TRUE);
        break;
      case 3:   // barrel rd (oo-o*)
        CalcWeaponPosition(FLOAT3D(fFX+fRDX, fFY+fRDY, 0), plLaserRail, TRUE);
        break;
    }
    // create laser projectile
    CEntityPointer penRail = CreateEntity(plLaserRail, CLASS_RAIL);
    // init and launch rail
    ELaunchRail eLaunch;
    eLaunch.penLauncher = m_penPlayer;
		eLaunch.bSniper = FALSE;
    penRail->Initialize(eLaunch);
  };

  // fire laser ray
  void FireSniperRail(void) {
    // rail start position
    CPlacement3D plSniperRail;
    CalcWeaponPosition(
      FLOAT3D(wpn_fFX[WEAPON_SNIPER],wpn_fFY[WEAPON_SNIPER], 0), 
      plSniperRail, TRUE);
    // create sniper projectile
    CEntityPointer penRail = CreateEntity(plSniperRail, CLASS_RAIL);
    // init and launch rail
    ELaunchRail eLaunch;
    eLaunch.penLauncher = m_penPlayer;
		eLaunch.bSniper = TRUE;
    penRail->Initialize(eLaunch);
  };

  // fire laser bomb
  void FireLaserBomb(void) {
    // laser start position
   CPlacement3D plLaserBomb;
    CalcWeaponPosition(
      FLOAT3D(wpn_fFX[WEAPON_LASER],wpn_fFY[WEAPON_LASER], 0), plLaserBomb, TRUE);
    // create laser projectile
    CEntityPointer penLaser = CreateEntity(plLaserBomb, CLASS_PROJECTILE);
    // init and launch laser projectile
    ELaunchProjectile eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.prtType = PRT_LASER_BOMB;
    penLaser->Initialize(eLaunch);
  };

  // fire cannon ball white
  void FireCannonBall(void)
  {
    // cannon ball start position
    CPlacement3D plBall;
    CalcWeaponPosition(
    FLOAT3D(wpn_fFX[WEAPON_IRONCANNON],wpn_fFY[WEAPON_IRONCANNON], 0), plBall, TRUE);
    // create cannon ball
  ...
  // fire cannon ball red
  void FireNukeBall(void)
  {
    // cannon ball start position
    CPlacement3D plBall;
    CalcWeaponPosition(
  ...
  // fire cannon ball blue
  void FireBlueBall(void)
  {
    // cannon ball start position
    CPlacement3D plBall;
    CalcWeaponPosition(
    ...
    eLaunch.fSize = 3.0f;
    eLaunch.fLaunchPower = 140.0f;
    eLaunch.cbtType = CBT_BLUE;
    penBall->Initialize(eLaunch);
  };

  // fire huge cannon ball
  void FireCannonHuge(INDEX iPower)
  {
    // cannon ball start position
    CPlacement3D plBall;
    CalcWeaponPosition(
    ...
    ELaunchCannonBall eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.fLaunchPower = 100.0f+iPower*5.0f; // ranges from 100-200 (since iPower can be max 20)
    eLaunch.fSize = 3.0f+iPower*0.15f;
    eLaunch.cbtType = CBT_IRON;
    penBall->Initialize(eLaunch);
  };
  ...
  void ThrowBomb(void) {
    ...
    ELaunchProjectile eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.prtType = PRT_BOMB;
    eLaunch.fSpeed = 60.0f;
    penGrenadeBomb->Initialize(eLaunch);
  };

  void FireTransProjectile(void) {
    CPlacement3D plTP;
    CalcWeaponPosition(
    FLOAT3D(wpn_fFX[WEAPON_COLT], wpn_fFY[WEAPON_COLT], 0), plTP, TRUE);
    // create trans projectile
    CEntityPointer penTP = CreateEntity(plTP, CLASS_TRANSPROJECTILE);
    // init and launch it
    ELaunchTP eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    m_penTP = penTP;
    penTP->Initialize(eLaunch);
  };

  // ghostbuster source
  void GetLightningRaySourcePlacement(CPlacement3D &plSource) {
    CalcWeaponPosition(
      FLOAT3D(wpn_fFX[WEAPON_COLT], wpn_fFY[WEAPON_COLT]-0.2f, 0), 
      plSource, TRUE);
  };

  // fire ghost buster ray
  void FireLightningRay(void) {
    // ray start position
    CPlacement3D plRay;
    GetLightningRaySourcePlacement(plRay);
    // fire ray
    ((CLightningRay&)*m_penGhostBusterRay).Fire(plRay);
  };
  ...
  void FireRail(void) {
    // rail start position
    CPlacement3D plRail;
    //CalcWeaponPosition(FLOAT3D(0, 0, 0), plRail, TRUE);
    CalcWeaponPosition(FLOAT3D(wpn_fFX[WEAPON_LASER],wpn_fFY[WEAPON_LASER], 0), plRail, TRUE);
    CEntityPointer penRail = CreateEntity(plRail, CLASS_RAIL);
    // init and launch rail
    ELaunchRail eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    penRail->Initialize(eLaunch);
  ...
  void FireTracer(void) {
    ...
    CPlacement3D plTracer;
		FLOAT fX = 0.0f;
		// switch barrels every other tracer
		if (m_iTracerCount == 1) {
			fX = 0.26f;
		}
		if (GetPlayer()->m_iViewState == PVT_3RDPERSONVIEW) { 
			CalcWeaponPosition(FLOAT3D(0.22f + fX, -0.68f, 0), plTracer, TRUE);
		} else {
			CalcWeaponPosition(FLOAT3D(-0.13f + fX, -0.18f, 0), plTracer, TRUE);
		}
  ...
  {
		//CPrintF("Player Weapons - iGiveWeapons: %d\n", iGiveWeapons);
    ...
    m_iAvailableWeapons &= WEAPONS_ALLAVAILABLEMASK;
		//CPrintF("Player Weapons - m_iAvailableWeapons: %d\n", m_iAvailableWeapons);
//    m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASK;

		if (GetSP()->sp_bMaskColt) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKCOLT;
		}
		if (GetSP()->sp_bMaskSingleShotgun) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKSINGLESHOTGUN;
		}
		if (GetSP()->sp_bMaskSniper) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKSNIPER;
		}
		if (GetSP()->sp_bMaskFlamer) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKFLAMER;
		}
		if (GetSP()->sp_bMaskTommyGun) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKTOMMYGUN;
		}
		if (GetSP()->sp_bMaskMiniGun) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKMINIGUN;
		}
		if (GetSP()->sp_bMaskRocketLauncher) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKROCKETLAUNCHER;
		}
		if (GetSP()->sp_bMaskGrenadeLauncher) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKGRENADELAUNCHER;
		}
		if (GetSP()->sp_bMaskLaser) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKLASER;
		}
		if (GetSP()->sp_bMaskCannon) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKIRONCANNON;
		}
		if (GetSP()->sp_bMaskChainsaw) {
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKCHAINSAW;
		}
		if (GetSP()->sp_bMaskTeleGun) {    
			m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASKDOUBLESHOTGUN;
		}
    ...
    ULONG ulNewWeapons = m_iAvailableWeapons&~ulOldWeapons;
		//CPrintF("Player Weapons - ulNewWeapons: %d\n", ulNewWeapons);
    // for each new weapon
    for(INDEX iWeapon=WEAPON_KNIFE; iWeapon<WEAPON_LAST; iWeapon++) {
			//CPrintF("Player Weapons: New Weapon, iWeapon: %d\n", iWeapon);
      if ( ulNewWeapons & (1<<(iWeapon-1)) ) {
				//CPrintF("Player Weapons: Give New Weapon, iWeapon: %d\n", iWeapon);
    ...
    m_iMaxBullets        = ClampUp((INDEX) ceil(MAX_BULLETS*fModifier),       INDEX(20000));
    m_iMaxShells         = ClampUp((INDEX) ceil(MAX_SHELLS*fModifier),        INDEX(20000));
    m_iMaxRockets        = ClampUp((INDEX) ceil(MAX_ROCKETS*fModifier),       INDEX(20000));
    m_iMaxGrenades       = ClampUp((INDEX) ceil(MAX_GRENADES*fModifier),      INDEX(20000));
    m_iMaxNapalm         = ClampUp((INDEX) ceil(MAX_NAPALM*fModifier),        INDEX(20000));
    m_iMaxElectricity    = ClampUp((INDEX) ceil(MAX_ELECTRICITY*fModifier),   INDEX(20000));
    m_iMaxIronBalls      = ClampUp((INDEX) ceil(MAX_IRONBALLS*fModifier),     INDEX(20000));
    m_iMaxSniperBullets  = ClampUp((INDEX) ceil(MAX_SNIPERBULLETS*fModifier), INDEX(20000));
    m_iMaxNukeBalls      = ClampUp((INDEX) ceil(MAX_NUKEBALLS*fModifier),     INDEX(20000));
  ...
  INDEX GetAmmo(void)
  {
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_COLT:            return 0;
      case WEAPON_DOUBLECOLT:      return 0;
      case WEAPON_SINGLESHOTGUN:   return m_iShells;
      case WEAPON_DOUBLESHOTGUN:   return 0;
      case WEAPON_TOMMYGUN:        return m_iNukeBalls;
  ...
  INDEX GetMaxAmmo(void)
  {
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_DOUBLESHOTGUN:   return 0;
      case WEAPON_TOMMYGUN:        return m_iMaxNukeBalls;
...
//    m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASK;
  ...
  void AddDefaultAmmoForWeapon(INDEX iWeapon, FLOAT fMaxAmmoRatio)
  {
    ...
    switch (iWeapon) {
      ...
      // bullets
      case WEAPON_TOMMYGUN:
        iAmmoPicked = (INDEX) Max(50.0f, m_iMaxNukeBalls*fMaxAmmoRatio);
...
//    m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASK;
    ...
    switch (wit) {
      case WIT_COLT:            
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("MP5K"), 0);
      ...
      case WIT_DOUBLESHOTGUN:   
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("?"), 0);
        fnmMsg = CTFILENAME("Data\\Messages\\Weapons\\doubleshotgun.txt"); 
        break;
      case WIT_TOMMYGUN:        
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Da Bomb"), 0);
      ...
      case WIT_SNIPER:        
        ...
        break;
      case WIT_MINIGUN:         
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("XM214-B Dual Minigun"), 0);
      ...
      case WIT_LASER:           
        ...
        break;
      case WIT_CANNON:          
        ((CPlayer&)*m_penPlayer).ItemPicked(TRANS("SBC 'Critter Cruncher' Cannon"), 0);
  ...
  BOOL ReceivePackAmmo(const CEntityEvent &ee)
  {
        ...
        (eapi.iNukeBalls>0 && m_iNukeBalls<m_iMaxNukeBalls))
    {
      ...
      if( eapi.iSniperBullets != 0) { strMessage.PrintF("%s %d %s,", (const char *) strMessage, eapi.iSniperBullets, TRANS("Sniper bullets")); iAmmoTypes++; }
			if( eapi.iNukeBalls != 0)     { strMessage.PrintF("%s %d %s,", (const char *) strMessage, eapi.iNukeBalls, TRANS("Da Bombs")); iAmmoTypes++; }
  ...
  {
		/*if (m_iCurrentWeapon == WEAPON_NONE || m_iCurrentWeapon == WEAPON_KNIFE) {
        ...
        WeaponSelectOk(WEAPON_KNIFE);
			...
			WeaponSelectOk(m_iCurrentWeapon);
		}*/
    switch (m_iCurrentWeapon) {
      case WEAPON_NONE: 
  ...
  BOOL HasAmmo(WeaponType EwtWeapon) {
    switch (EwtWeapon) {
      ...
      case WEAPON_DOUBLESHOTGUN: return TRUE;
      case WEAPON_TOMMYGUN: return (m_iNukeBalls>0);
  ...
  // does weapon have ammo for secondary fire
  BOOL HasAmmoSecondary(WeaponType EwtWeapon) {
    switch (EwtWeapon) {
      case WEAPON_KNIFE: case WEAPON_COLT: case WEAPON_DOUBLECOLT: return TRUE;
      case WEAPON_SINGLESHOTGUN: return (m_iShells>0);
      case WEAPON_DOUBLESHOTGUN: return TRUE;
      ...
      case WEAPON_SNIPER: return (m_iSniperBullets>0);
      case WEAPON_MINIGUN: return (m_iBullets>0);
      case WEAPON_ROCKETLAUNCHER: return (m_iRockets>1);
      case WEAPON_GRENADELAUNCHER: return (m_iGrenades>1);
      case WEAPON_FLAMER: return (m_iNapalm>4);
      case WEAPON_CHAINSAW: return TRUE;
      case WEAPON_LASER: return (m_iElectricity>0);
      case WEAPON_IRONCANNON: return (m_iIronBalls>0);
    }
  ...
  void PlayDefaultAnim(void) {
    switch(m_iCurrentWeapon) {
      ...
      case WEAPON_DOUBLECOLT:
        ...
        m_moWeaponSecond.PlayAnim(COLT_ANIM_WAIT1, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
      case WEAPON_SINGLESHOTGUN:
        m_moWeapon.PlayAnim(SINGLESHOTGUN_ANIM_WAIT1, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
      case WEAPON_DOUBLESHOTGUN:
        m_moWeapon.PlayAnim(GUNHAND_ANIM_WAIT1, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
      case WEAPON_TOMMYGUN:
        m_moWeapon.PlayAnim(BOMB_ANIM_WAIT, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
			/*case WEAPON_TOMMYGUN:
        m_moWeapon.PlayAnim(MINI1GUN_ANIM_WAIT1, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;*/
      case WEAPON_SNIPER:
        m_moWeapon.PlayAnim(SNIPER_ANIM_WAIT01, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
      case WEAPON_MINIGUN:
        m_moWeapon.PlayAnim(MINI2GUN_ANIM_WAIT1, AOF_LOOPING|AOF_NORESTART|AOF_SMOOTHCHANGE); break;
      case WEAPON_ROCKETLAUNCHER:
...
//      case WEAPON_NUKECANNON:
  ...
  FLOAT DoubleShotgunBoring(void) {
    ...
    INDEX iAnim;
    switch (IRnd()%2) {
      case 0: iAnim = GUNHAND_ANIM_WAIT3; break;
      case 1: iAnim = GUNHAND_ANIM_WAIT4; break;
    }
    m_moWeapon.PlayAnim(iAnim, AOF_SMOOTHCHANGE);
    return m_moWeapon.GetAnimLength(iAnim);
  };
  ...
  FLOAT TommyGunBoring(void) {
    ...
    INDEX iAnim;
    switch (IRnd()%3) {
      case 0: iAnim = BOMB_ANIM_WAIT; break;
    }
    m_moWeapon.PlayAnim(iAnim, AOF_SMOOTHCHANGE);
    return m_moWeapon.GetAnimLength(iAnim);
  };
  ...
  FLOAT TommyGunBoring(void) {
    ...
    INDEX iAnim;
    switch (IRnd()%3) {
      case 0: iAnim = MINI1GUN_ANIM_WAIT2; break;
      case 1: iAnim = MINI1GUN_ANIM_WAIT3; break;
      case 2: iAnim = MINI1GUN_ANIM_WAIT4; break;
    }
    m_moWeapon.PlayAnim(iAnim, AOF_SMOOTHCHANGE);
    return m_moWeapon.GetAnimLength(iAnim);
  };
*/
  ...
  FLOAT MiniGunBoring(void) {
    ...
    INDEX iAnim;
    switch (IRnd()%3) {
      case 0: iAnim = MINI2GUN_ANIM_WAIT2; break;
      case 1: iAnim = MINI2GUN_ANIM_WAIT3; break;
      case 2: iAnim = MINI2GUN_ANIM_WAIT4; break;
    }
    m_moWeapon.PlayAnim(iAnim, AOF_SMOOTHCHANGE);
    return m_moWeapon.GetAnimLength(iAnim);
  };
  ...
  PutDown() {
    ...
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_SINGLESHOTGUN:
        ...
        break;
      case WEAPON_DOUBLESHOTGUN:
        m_iAnim = GUNHAND_ANIM_DEACTIVATE;
        break;
      case WEAPON_TOMMYGUN:
        m_iAnim = BOMB_ANIM_DEACTIVATE;
        break;
/*      case WEAPON_TOMMYGUN:
        m_iAnim = MINI1GUN_ANIM_DEACTIVATE;
        break;*/
      case WEAPON_SNIPER:
        m_iAnim = SNIPER_ANIM_DEACTIVATE;
        break;
      case WEAPON_MINIGUN:
        m_iAnim = MINI2GUN_ANIM_DEACTIVATE;
        break;
      case WEAPON_ROCKETLAUNCHER:
  ...
  BringUp() {
    ...
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_SINGLESHOTGUN:
        ...
        break;
      case WEAPON_DOUBLESHOTGUN:
        m_iAnim = GUNHAND_ANIM_ACTIVATE;
        //SetFlare(0, FLARE_REMOVE);
        break;
			case WEAPON_TOMMYGUN:
				m_iAnim = BOMB_ANIM_ACTIVATE;
				break;
/*      case WEAPON_TOMMYGUN: {
        CAttachmentModelObject *amo = m_moWeapon.GetAttachmentModel(MINI1GUN_ATTACHMENT_BARRELS);
        m_aMiniGunLast = m_aMiniGun = amo->amo_plRelative.pl_OrientationAngle(3);
        m_iAnim = MINI1GUN_ANIM_ACTIVATE;
        SetFlare(0, FLARE_REMOVE);
        break;}*/
      case WEAPON_SNIPER:
        ...
        break;
      case WEAPON_MINIGUN: {
        CAttachmentModelObject *amo = m_moWeapon.GetAttachmentModel(MINI2GUN_ATTACHMENT_BARRELS01);
        m_aMiniGunLast = m_aMiniGun = amo->amo_plRelative.pl_OrientationAngle(3);
        //CAttachmentModelObject *amo2 = m_moWeapon.GetAttachmentModel(MINI2GUN_ATTACHMENT_BARRELS02);
        //m_aMiniGunLast = m_aMiniGun = amo2->amo_plRelative.pl_OrientationAngle(3);
        m_iAnim = MINI2GUN_ANIM_ACTIVATE;
        SetFlare(0, FLARE_REMOVE);
        SetFlare(1, FLARE_REMOVE);
        break; }
      case WEAPON_ROCKETLAUNCHER:
...
/*
  ...
  Fire()
  {
    ...
    if (m_iCurrentWeapon==WEAPON_MINIGUN) {
      ...
      jump ChainsawFire();
    /*} else if (m_iCurrentWeapon==WEAPON_DOUBLESHOTGUN) {
...
//    }
      ...
      wait() {
        on (EBegin) : {
          ...
          switch (m_iCurrentWeapon) {
            ...
            //case WEAPON_DOUBLESHOTGUN: call FireRail(); break;
            case WEAPON_DOUBLESHOTGUN: call FireTP(); break;
            //case WEAPON_DOUBLESHOTGUN: call FireGhostBuster(); break;
						case WEAPON_TOMMYGUN:	call FireBomb(); break;
            case WEAPON_SNIPER: call FireSniper(); break;
            //case WEAPON_SNIPER: call FireRail(); break;
            ...
            case WEAPON_LASER: call FireLaser(); break;
						case WEAPON_IRONCANNON: call FireCannon(); break;
            default: ASSERTALWAYS("Unknown weapon.");
          }
          resume;
        }
    ...
    switch (m_iCurrentWeapon) {
//      case WEAPON_TOMMYGUN: { jump TommyGunSpinDown(); break; }
      case WEAPON_COLT: { jump ColtStop(); break; }
...
//      case WEAPON_TOMMYGUN: { jump TommyGunStop(); break; }
      ...
      //case WEAPON_DOUBLESHOTGUN: { jump GhostBusterStop(); break; }
      case WEAPON_LASER: { 
  ...
  FireSecondary()
  {
		...
		m_bHasAmmo = HasAmmoSecondary(m_iCurrentWeapon);
		...
		while (HoldingSecondaryFire()) {
			...
			wait() {
				on (EBegin) : {
					// throw one knife
					switch (m_iCurrentWeapon) {
						case WEAPON_KNIFE: call ThrowKnife(); break;
						case WEAPON_CHAINSAW: call NullSecondaryFire(); break;
						case WEAPON_ROCKETLAUNCHER: call FireSecondaryRocket(); break;
						case WEAPON_COLT: call NullSecondaryFire(); break;
						case WEAPON_DOUBLECOLT: call NullSecondaryFire(); break;
						case WEAPON_SINGLESHOTGUN: call NullSecondaryFire(); break;
						case WEAPON_DOUBLESHOTGUN:  call TeleportPlayer(); break;
						case WEAPON_TOMMYGUN: call NullSecondaryFire(); break;
						case WEAPON_MINIGUN: call NullSecondaryFire(); break;
						case WEAPON_GRENADELAUNCHER: call FireGrenadeBomb(); break;
						//case WEAPON_LASER: call LaserSecondaryFire(); break;
						case WEAPON_LASER: call NullSecondaryFire(); break;
						//case WEAPON_IRONCANNON: call NullSecondaryFire(); break;
						//case WEAPON_IRONCANNON: jump FireAltCannon(); break;
						case WEAPON_SNIPER: call NullSecondaryFire(); break;
						//case WEAPON_SNIPER: call FireExplosiveSniper(); break;
						case WEAPON_FLAMER: call FlamerSecondaryFire(); break;
						default: { call FlamerSecondaryFire(); }
					}
					resume;
				}
				on (EEnd) : { 
...
//*/    
  ...
  SwingKnife() {
    ...
    m_moWeapon.PlayAnim(m_iAnim, 0);
    CutWithKnife(0, 0, /*range=*/6.0f, /*wide=*/6.0f, /*thickness=*/3.0f, /*damage=*/60.0f*m_fDamageAmount);
    autowait(0.4f/m_fRate);
    return EEnd();
  };
...
// ***************** FIRE COLT *****************
  ColtStart() {
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    PlaySound(pl.m_soWeapon0, SOUND_SILENCE, SOF_3D|SOF_VOLUMETRIC);      // stop possible sounds
    pl.m_soWeapon0.Set3DParameters(30.0f, 3.0f, 1.5f, 1.0f);       // fire
    PlaySound(pl.m_soWeapon0, SOUND_COLT_FIRE, SOF_LOOP|SOF_3D|SOF_VOLUMETRIC);
    return EEnd();
  };

  ColtStop() {
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    pl.m_soWeapon0.Set3DParameters(30.0f, 3.0f, 0.0f, 1.0f);      // mute fire
		GetAnimator()->FireAnimation(BODY_ANIM_COLT_FIRERIGHT, 0);  // play right 3rd animation
		m_moWeapon.PlayAnim(COLT_ANIM_FIRE2, AOF_LOOPING|AOF_NORESTART);
    jump Idle();
  };

  FireColt() {
		FireOneBullet(wpn_fFX[WEAPON_COLT], wpn_fFY[WEAPON_COLT], 500.0f, 15.0f*m_fDamageAmount);
    SpawnRangeSound(50.0f);
    SetFlare(0, FLARE_ADD);
    m_moWeapon.PlayAnim(COLT_ANIM_FIRE1, AOF_LOOPING|AOF_NORESTART);
    // firing FX
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    ...
    //((CPlayerView&)*m_pen3rdPersonView).m_fDistance>2.0f
    //if (pl.m_penCamera==NULL && pl.m_pen3rdPersonView==NULL && pl.m_pen3rdPersonView.m_fDistance>2.0f) {
    if (pl.m_penCamera==NULL && pl.m_pen3rdPersonView==NULL) {
       CalcWeaponPosition(FLOAT3D(afTommygunShellPos[0], afTommygunShellPos[1], afTommygunShellPos[2]), plShell, FALSE);
    // if 3rd person view
    } else {
       CalcWeaponPosition(FLOAT3D(afTommygunShellPos3rd[0], afTommygunShellPos3rd[1], afTommygunShellPos3rd[2]), plShell, FALSE);
	  }
    FLOATmatrix3D mRot;
    MakeRotationMatrixFast(mRot, plShell.pl_OrientationAngle);
    if( hud_bShowWeapon && !m_bSniping) {
      // empty bullet shell
      ...
      sld.sld_estType = ESL_COLT_SHELLS;  
      pl.m_iFirstEmptySLD = (pl.m_iFirstEmptySLD+1) % MAX_FLYING_SHELLS;
  ...
  DoubleColtStart() {
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    PlaySound(pl.m_soWeapon0, SOUND_SILENCE, SOF_3D|SOF_VOLUMETRIC);      // stop possible sounds
		pl.m_soWeapon0.Set3DParameters(50.0f, 5.0f, 1.75f, 1.0f);        // fire
    PlaySound(pl.m_soWeapon0, SOUND_COLT_FIRE2, SOF_LOOP|SOF_3D|SOF_VOLUMETRIC);
    return EEnd();
  };
  ...
  DoubleColtStop() {
    ...
    pl.m_soWeapon0.Set3DParameters(50.0f, 5.0f, 0.0f, 1.0f);      // mute fire
		GetAnimator()->FireAnimation(BODY_ANIM_COLT_FIRERIGHT, 0);  // play right 3rd animation
		m_moWeapon.PlayAnim(COLT_ANIM_FIRE2, AOF_LOOPING|AOF_NORESTART); // play right 1st animation
		autowait(0.1f);  // wait a bit
    GetAnimator()->FireAnimation(BODY_ANIM_COLT_FIRELEFT, 0);  // play left 3rd animation
		m_moWeaponSecond.PlayAnim(COLT_ANIM_FIRE2, AOF_LOOPING|AOF_NORESTART);   // play left 1st colt anim
    jump Idle();
  };

  FireDoubleColt() {
		// fire first colt
    SpawnRangeSound(75.0f);
    SetFlare(0, FLARE_ADD);
		//FireOneBullet(wpn_fFX[WEAPON_DOUBLECOLT], wpn_fFY[WEAPON_DOUBLECOLT], 500.0f, 20.0f*m_fDamageAmount);
    FireMachineBullet(wpn_fFX[WEAPON_DOUBLECOLT], wpn_fFY[WEAPON_DOUBLECOLT], 500.0f, 15.0f*m_fDamageAmount, 0.01f, 0.1f);
    m_moWeapon.PlayAnim(COLT_ANIM_FIRE1, AOF_LOOPING|AOF_NORESTART);  // player left colt 1st anim
    // firing FX
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    CPlacement3D plShell;
    //CalcWeaponPosition(FLOAT3D(afTommygunShellPos[0], afTommygunShellPos[1], afTommygunShellPos[2]), plShell, FALSE);
    ...
    autowait(0.025f/m_fRate);
		...
		//FireOneBullet(wpn_fFX[WEAPON_DOUBLECOLT], wpn_fFY[WEAPON_DOUBLECOLT], 500.0f, 20.0f*m_fDamageAmount);
    FireMachineBullet(wpn_fFX[WEAPON_DOUBLECOLT], wpn_fFY[WEAPON_DOUBLECOLT], 500.0f, 15.0f*m_fDamageAmount, 0.01f, 0.1f);
		m_moWeaponSecond.PlayAnim(COLT_ANIM_FIRE1, AOF_LOOPING|AOF_NORESTART);   // play right colt 1st anim
    // firing FX
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    CPlacement3D plShell;
    //CalcWeaponPosition(FLOAT3D(afTommygunShellPosLeft[0], afTommygunShellPosLeft[1], afTommygunShellPosLeft[2]), plShell, FALSE);
    if (pl.m_penCamera==NULL && pl.m_pen3rdPersonView==NULL) {
       CalcWeaponPosition(FLOAT3D(afTommygunShellPosLeft[0], afTommygunShellPosLeft[1], afTommygunShellPosLeft[2]), plShell, FALSE);
    // if 3rd person view
    } else {
       CalcWeaponPosition(FLOAT3D(afTommygunShellPosLeft3rd[0], afTommygunShellPosLeft3rd[1], afTommygunShellPosLeft3rd[2]), plShell, FALSE);
	  }
    FLOATmatrix3D mRot;
    MakeRotationMatrixFast(mRot, plShell.pl_OrientationAngle);
    if( hud_bShowWeapon && !m_bSniping) {
  ...
  FireSingleShotgun() {
    ...
    if (m_iShells>0) {
      ...
      FireBullets(wpn_fFX[WEAPON_SINGLESHOTGUN], wpn_fFY[WEAPON_SINGLESHOTGUN], 
        500.0f, 7.0f*m_fDamageAmount, 7, afSingleShotgunPellets, 0.2f, 0.06f);
      DecAmmo(m_iShells, 1);
...
//      PlayLightAnim(LIGHT_ANIM_COLT_SHOTGUN, 0);
      ...
      // sound
      CPlayer &pl = (CPlayer&)*m_penPlayer;
			pl.m_soWeapon0.Set3DParameters(25.0f, 3.0f, 0.75f, 1.0f);       // fire
  ...
  FireTP() {
    if (m_penTP==NULL) {
      //GetAnimator()->FireAnimation(BODY_ANIM_COLT_STAND, 0);
		  m_moWeapon.PlayAnim((GUNHAND_ANIM_FIRE1), 0);
		  FireTransProjectile();
      // sound
      SpawnRangeSound(20.0f);
      CPlayer &pl = (CPlayer&)*m_penPlayer;
      pl.m_soWeapon1.Set3DParameters(10.0f, 5.0f, 0.5f, 1.0f);
  ...
  TeleportPlayer() {
    if (m_penTP!=NULL) {
			if (GetSP()->sp_bControlRunning) {
				...
				if (((CMovableEntity&)*m_penTP).en_vCurrentTranslationAbsolute.Length()<0.1f) {
					m_penTP->SendEvent(ETrigger());
					m_penTP = NULL;
				}
			} else {
				m_penTP->SendEvent(ETrigger());
  ...
  FireRail() {
    if (m_iSniperBullets>0) {
      GetAnimator()->FireAnimation(BODY_ANIM_SHOTGUN_FIRELONG, 0);
		  m_moWeapon.PlayAnim(SNIPER_ANIM_WAIT01, 0);
		  FireRail();
  ...
  GhostBusterStart() {
    // create ray
    m_penGhostBusterRay = CreateEntity(GetPlacement(), CLASS_LIGHTNINGRAY);
    ELightningRay elr;
    elr.penOwner = this;
    m_penGhostBusterRay->Initialize(elr);
    // play anim
    m_moWeapon.PlayAnim((GUNHAND_ANIM_FIRE1), 0);
    // play fire sound
    //CPlayer &pl = (CPlayer&)*m_penPlayer;
  ...
  GhostBusterStop() {
    //GetAnimator()->FireAnimationOff();
    // destroy ray
    ((CLightningRay&)*m_penGhostBusterRay).DestroyLightningRay();    
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    //pl.m_soWeapon0.Stop();
    jump Idle();
  };
  ...
  FireGhostBuster() {
	  ...
	  FireBomb() 
	{
		if (m_iNukeBalls>0) {
			GetAnimator()->FireAnimation(BODY_ANIM_KNIFE_ATTACK, 0);
			m_moWeapon.PlayAnim((BOMB_ANIM_THROW), 0);
			ThrowBomb();
			...
			pl.m_soWeapon1.Set3DParameters(10.0f, 5.0f, 0.5f, 1.0f);
			PlaySound(pl.m_soWeapon1, SOUND_GRENADELAUNCHER_FIRE, SOF_3D|SOF_VOLUMETRIC);
			autowait(0.5f/m_fRate);
      if (m_iNukeBalls<=0) {
        SelectNewWeapon();
      }
    } else {
      ASSERTALWAYS("Da Bomb - Auto weapon change not working.");
      m_bFireWeapon = m_bHasAmmo = FALSE;
    }
    return EEnd();
  };
  ...
  FireSniper() {
    if (m_iSniperBullets>0) {
      // fire one bullet
      //FireSniperBullet(wpn_fFX[WEAPON_SNIPER], wpn_fFY[WEAPON_SNIPER], 1500.0f, 400.0f);
      //FireSniperBullet(0.0f, 0.0f, 1500.0f, 400.0f*m_fDamageAmount, 0.0f);
			FireSniperRail();
      DecAmmo(m_iSniperBullets, 1);
      if (!m_bSniping) { SetFlare(0, FLARE_ADD); }
      // sound
      CPlayer &pl = (CPlayer&)*m_penPlayer;
      pl.m_soWeapon0.Set3DParameters(35.0f, 4.0f, 1.5f, 1.0f);
      PlaySound(pl.m_soWeapon0, SOUND_SNIPER_FIRE, SOF_3D|SOF_VOLUMETRIC);
	...
	FireExplosiveSniper() {
      // fire one bullet
      FireExplosiveBullet(0.0f, 0.0f, 1500.0f, 300.0f, 0.0f);
      if (!m_bSniping) { SetFlare(0, FLARE_ADD); }
      // sound
      CPlayer &pl = (CPlayer&)*m_penPlayer;
      pl.m_soWeapon0.Set3DParameters(50.0f, 4.0f, 1.75f, 1.0f);
      PlaySound(pl.m_soWeapon0, SOUND_SNIPER_FIRE, SOF_3D|SOF_VOLUMETRIC);
      if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("SniperFire");}
      // animation
  ...
  MiniGunSpinUp() {
    // steady anim
    m_moWeapon.PlayAnim(MINI2GUN_ANIM_WAIT1, AOF_LOOPING|AOF_NORESTART);
    // no boring animation
    ((CPlayerAnimator&)*((CPlayer&)*m_penPlayer).m_penAnimator).m_fLastActionTime = _pTimer->CurrentTick();
    // clear last lerped bullet position
    m_iLastBulletPosition = FLOAT3D(32000.0f, 32000.0f, 32000.0f);
  ...
  MiniGunFire() {
    ...
    if(HoldingFire() && m_iBullets>0) {
      // play fire sound
      PlaySound(pl.m_soWeapon0, SOUND_MINIGUN_FIRE2, SOF_3D|SOF_LOOP|SOF_VOLUMETRIC);
    ...
    while (HoldingFire()) {
      ...
      if (m_iBullets>0) {
        // fire a bullet
        FireMachineBullet(wpn_fFX[WEAPON_MINIGUN], wpn_fFY[WEAPON_MINIGUN], 750.0f, 30.0f*m_fDamageAmount, 0.01f, 0.2f);
      ...
      autowait(MINIGUN_TICKTIME/m_fRate*0.5f);
			FireMachineBullet(wpn_fFX[WEAPON_MINIGUN], wpn_fFY[WEAPON_MINIGUN], 750.0f, 30.0f*m_fDamageAmount, 0.01f, 0.2f);
			autowait(MINIGUN_TICKTIME/m_fRate*0.5f);
      // spin
      m_aMiniGunLast = m_aMiniGun;
      m_aMiniGun+=m_aMiniGunSpeed*MINIGUN_TICKTIME*2;
    }
    GetAnimator()->FireAnimationOff();
    // stop fire sound
    CPlayer &pl = (CPlayer&)*m_penPlayer;
  ...
  MiniGunSpinDown() {
    ...
    while ( m_aMiniGunSpeed>0 && (!HoldingFire() || m_iBullets<=0)) {
      autowait(MINIGUN_TICKTIME/m_fRate);
      // spin
      m_aMiniGunLast = m_aMiniGun;
      m_aMiniGun+=m_aMiniGunSpeed*MINIGUN_TICKTIME*2;
      m_aMiniGunSpeed-=MINIGUN_SPINDNACC*MINIGUN_TICKTIME*2;
      ...
      // fire one rocket
        ...
        DecAmmo(m_iRockets, 1);
			  ...
			  SpawnRangeSound(5.0f);
				pl.m_soWeapon0.Set3DParameters(0.0f, 0.0f, 0.1f, 1.0f);
				PlaySound(pl.m_soWeapon1, SOUND_ROCKETLAUNCHER_FIRE, SOF_3D|SOF_VOLUMETRIC);
			  CModelObject *pmo1 = &(m_moWeapon.GetAttachmentModel(ROCKETLAUNCHER_ATTACHMENT_ROCKET1)->amo_moModelObject);
			  pmo1->StretchModel(FLOAT3D(0, 0, 0));
			  autowait(0.01f);
			  CModelObject *pmo2 = &(m_moWeapon.GetAttachmentModel(ROCKETLAUNCHER_ATTACHMENT_ROCKET1)->amo_moModelObject);
			  pmo2->StretchModel(FLOAT3D(1, 1, 1));
			  autowait(0.2f/m_fRate);
        // no ammo -> change weapon
  ...
  FireSecondaryRocket() {
    if (m_iRockets>1) {
      ...
      /*if( m_penRayHit!=NULL) {
        ...
        if( pen->GetFlags()&ENF_ALIVE ) {
				  if ( IsOfClass(pen, "Player") ) {
					  ...
					  m_tmLastTarget = _pTimer->CurrentTick()+2.0f;
				  } else if( IsDerivedFromClass(pen, "Enemy Base") ) {
					  // set up the target
					  m_penRocketLauncherHoming = pen;
					  m_strLastTarget.PrintF(TRANS("Locked on %s"), (const char *) ((CEnemyBase*)pen)->GetIngameName());
					  m_tmLastTarget = _pTimer->CurrentTick()+2.0f;
				  } else {
      ...
      ASSERTALWAYS("RocketLauncher - Auto weapon change not working.");
      m_bSecondaryFire = m_bHasAmmo = FALSE;
    }
    return EEnd();
  };
  ...
  FireGrenadeLauncher()
  {
    if (m_iGrenades>0) {
      ...
      CPlayer &pl = (CPlayer&)*m_penPlayer;
      PlaySound(pl.m_soWeapon1, SOUND_GRENADELAUNCHER_FIRE, SOF_3D|SOF_VOLUMETRIC);
      autowait(0.3f/m_fRate);
      // no ammo -> change weapon
      if (m_iGrenades<=0) { SelectNewWeapon(); }
    } else {
  ...
  FireGrenadeBomb() 
    ...
    if (m_iGrenades>1) {
      GetAnimator()->FireAnimation(BODY_ANIM_MINIGUN_FIRELONG, 0);
		  m_moWeapon.PlayAnim((GRENADELAUNCHER_ANIM_FIRE), 0);
		  FireGrenadeBomb();
      DecAmmo(m_iGrenades, 2);
      // sound
      SpawnRangeSound(20.0f);
      CPlayer &pl = (CPlayer&)*m_penPlayer;
      pl.m_soWeapon1.Set3DParameters(40.0f, 5.0f, 2.0f, 1.0f);
      PlaySound(pl.m_soWeapon1, SOUND_GRENADELAUNCHER_FIRE, SOF_3D|SOF_VOLUMETRIC);
      autowait(0.5f/m_fRate);
      // no ammo -> change weapon
      if (m_iGrenades<=1) { SelectNewWeapon(); }
    } else {
      ASSERTALWAYS("GrenadeLauncher - Auto weapon change not working.");
      m_bFireWeapon = m_bHasAmmo = FALSE;
    }
    ...
    if (m_iNapalm>4) {
      ...
      /*if( m_penRayHit!=NULL) {
        ...
        if( pen->GetFlags()&ENF_ALIVE ) {
		  ...
		  // fire one flamer projectile
      m_moWeapon.PlayAnim(FLAMER_ANIM_FIREEND, 0);
		  if(m_penFlamerProjectile != NULL ) {
			  if( !(m_penFlamerProjectile->GetFlags()&ENF_ALIVE) ) {
				  m_penFlamerProjectile = NULL;
			  }
		  }
      FireFlamerProjectile();
      DecAmmo(m_iNapalm, 5);
      // play fire sound
      SpawnRangeSound(50.0f);
      CPlayer &pl = (CPlayer&)*m_penPlayer;
      ...
      autowait(0.75f/m_fRate);
      // no ammo -> change weapon
      if (m_iNapalm<=4) { SelectNewWeapon(); }
    } else {
      ASSERTALWAYS("GrenadeLauncher - Auto weapon change not working.");
      m_bSecondaryFire = m_bHasAmmo = FALSE;
    }
  ...
  ChainsawFire() {
    ...
    while (HoldingFire())// && m_iNapalm>0)
    {
      autowait(CHAINSAW_UPDATETIME/m_fRate);
      // 200 damage per second
      CutWithChainsaw(0, 0, 5.0f, 5.0f, 2.0f, 800.0f*CHAINSAW_UPDATETIME*m_fDamageAmount);
      //InflictDirectDamage(penClosest, m_penPlayer, DMT_CHAINSAW, fDamage, vHit, vDir);
    }
...
//    PlaySound(pl.m_soWeapon0, SOUND_SILENCE, SOF_3D|SOF_VOLUMETRIC/*|SOF_SMOOTHCHANGE*/);
 ...
 LaserSecondaryFire() 
 {
    if (m_iElectricity>0) {
      ...
      PlaySound(pl.m_soWeapon0, SOUND_LASER_FIRE, SOF_3D|SOF_VOLUMETRIC);
		  // animations
      CModelObject *pmo1 = &(m_moWeapon.GetAttachmentModel(LASER_ATTACHMENT_LEFTUP)->amo_moModelObject);
      CModelObject *pmo2 = &(m_moWeapon.GetAttachmentModel(LASER_ATTACHMENT_RIGHTDOWN)->amo_moModelObject);
      CModelObject *pmo3 = &(m_moWeapon.GetAttachmentModel(LASER_ATTACHMENT_LEFTDOWN)->amo_moModelObject);
      CModelObject *pmo4 = &(m_moWeapon.GetAttachmentModel(LASER_ATTACHMENT_RIGHTUP)->amo_moModelObject);
      pmo1->PlayAnim(BARREL_ANIM_FIRE, 0);
      pmo2->PlayAnim(BARREL_ANIM_FIRE, 0);
      pmo3->PlayAnim(BARREL_ANIM_FIRE, 0);
      pmo4->PlayAnim(BARREL_ANIM_FIRE, 0);
      autowait(0.375f/m_fRate);
      // no ammo -> change weapon
      if (m_iElectricity<=1) { SelectNewWeapon(); }
    } else {
      ASSERTALWAYS("Laser - Auto weapon change not working.");
      m_bSecondaryFire = m_bHasAmmo = FALSE;
    }
 ...
 // ***************** FIRE CANNON *****************
  FireCannon() {
    if (m_iIronBalls>0) {
	    m_iCannonThang++;
	    // fire 3 diff skinned balls
      GetAnimator()->FireAnimation(BODY_ANIM_MINIGUN_FIRESHORT, 0);
      m_moWeapon.PlayAnim(CANNON_ANIM_FIRE, 0);
	    if (m_iCannonThang==1) {
        FireNukeBall(); // fire red ball
	    } else if (m_iCannonThang==2) {
		    FireCannonBall(); // fire white ball
	    } else {
        FireBlueBall(); // fire blue ball
	      m_iCannonThang = 0;
	    }
  ...
  FireAltCannon()
  {
    ...
    if (m_iIronBalls>0) {
			...
			while (HoldingSecondaryFire() && ((_pTimer->CurrentTick()-TM_START)<0.5f) ) {
				autowait(_pTimer->TickQuantum);
				//INDEX iPower = INDEX((_pTimer->CurrentTick()-TM_START)*2.0f);
				INDEX iPower = INDEX((_pTimer->CurrentTick()-TM_START)/_pTimer->TickQuantum*2.0f);
      ...
      if (GetSP()->sp_bCooperative) {
        ...
        fFalloff = 30.0f;
			...
			INDEX iPower = INDEX((_pTimer->CurrentTick()-TM_START)*100.0f);
			//CPrintF("iPower: %d\n", iPower);
      FireCannonHuge(iPower);
...
//    PlayLightAnim(LIGHT_ANIM_NONE, 0);
  ...
  Main(EWeaponsInit eInit) {
    ...
    SetCollisionFlags(ECF_IMMATERIAL);
		...
		// get stupid with the fire rate
		m_fRate = GetSP()->sp_fWeaponsFireRate;
		if (m_fRate<=0) {
			m_fRate = 0.01f;
		}
